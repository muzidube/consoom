...
export default function TV({ name, poster_path, vote_average, first_air_date, id }) {
  ...
  return (
    <div className="media-item tv ml-5 w-150px min-w-150px bg-transparent mt-0 border-none shadow-none rounded-lg overflow-visible mt-0 relative top-0 left-0 flex flex-wrap content-start whitespace-nowrap">
      <Link to={`/TV/${id}/${name}`}>
      ...
      <div className="tv-info pt-6 pb-0 px-2.5 relative whitespace-normal grid content-start w-full">
        ...
        <p className="m-0 p-0 whitespace-normal font-normal">{formatDate(first_air_date)}</p>
      </div>
    </div>
  );
}
