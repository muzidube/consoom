...
module.exports.fiction = app.get('/bookAPI/popular/fiction', (request, response) => {
  ...
  axios(config)
    .then((fictionResponse) => {
      response.json(JSON.stringify(fictionResponse.results.books));
    })
    .catch((error) => {
      console.log(error);
    });
...
module.exports.nonFiction = app.get('/bookAPI/non-fiction', (request, response) => {
  const config = {
    method: 'get',
  ...
  axios(config)
    .then((nonFictionResponse) => {
      response.json(JSON.stringify(nonFictionResponse.results.books));
    })
    .catch((error) => {
      console.log(error);
    });
